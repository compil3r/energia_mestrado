...
[startBtn, howtoBtn, exitBtn].forEach(btn => {
...
exitBtn.addEventListener('click', () => {
  document.body.innerHTML = '<h2 style="color:#eaeaea;text-align:center;margin-top:20vh;">Obrigado por jogar!</h2>';
  bgSound.pause();
});

// Título com efeito de flicker
const titleText = 'A Cidade sem Energia';
const titleElem = document.getElementById('game-title');
function renderTitleFlicker() {
  // Letras que podem piscar: só as do meio, não espaços
  let html = '';
  for (let i = 0; i < titleText.length; i++) {
    const char = titleText[i];
    if (char !== ' ' && Math.random() < 0.18) {
      html += `<span class="letter-flicker">${char}</span>`;
    } else {
      html += `<span>${char}</span>`;
    }
  }
...
setInterval(renderTitleFlicker, 350);
renderTitleFlicker();

// Cidade minimalista com prédios estáticos e janelas animadas
